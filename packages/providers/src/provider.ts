...
import type { Bech32Address, ContractIdLike } from '@fuel-ts/interfaces';
...
export default class Provider {
  ...
  async getCoins(
    ...
    const result = await this.operations.getCoins({
      first: 10,
      ...paginationArgs,
      filter: { owner: Bech32.toB256FromString(owner), assetId: assetId && hexlify(assetId) },
    });
    ...
    return coins.map((coin) => ({
      ...
      owner: Bech32.fromString(coin.owner),
  ...
  async getResourcesToSpend(
    ...
    const result = await this.operations.getResourcesToSpend({
      owner: Bech32.toB256FromString(owner),
    ...
    return result.resourcesToSpend.flat().map((resource) => {
      if (isRawCoin(resource)) {
        return {
          ...
          owner: Bech32.fromString(resource.owner),
      ...
      return {
        sender: Bech32.fromString(resource.sender),
        recipient: Bech32.fromString(resource.recipient),
  ...
  async getContractBalance(
    ...
    const { contractBalance } = await this.operations.getContractBalance({
      contract: Bech32.toB256FromString(contractId),
  ...
  async getBalances(
    ...
    const result = await this.operations.getBalances({
      first: 10,
      ...paginationArgs,
      filter: { owner: Bech32.toB256FromString(owner) },
    });
  ...
  async getMessages(
    ...
    const result = await this.operations.getMessages({
      first: 10,
      ...paginationArgs,
      owner: Bech32.toB256FromString(address),
    });

    const messages = result.messages.edges!.map((edge) => edge!.node!);

    return messages.map((message) => ({
      sender: Bech32.fromString(message.sender),
      recipient: Bech32.fromString(message.recipient),
  ...
  async getMessageProof(
    ...
    return {
      ...
      sender: Bech32.fromString(result.messageProof.sender),
      recipient: Bech32.fromString(result.messageProof.recipient),
